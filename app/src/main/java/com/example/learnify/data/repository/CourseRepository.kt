...
class CourseRepository(
    ...
    suspend fun searchAndSave(
        ...
        return try {
            val playlists = withContext(Dispatchers.IO) {
                RetrofitInstance.api.searchPlaylists(
                    query = query,
                    apiKey = apiKey
                )
            }
            ...
            val courses = playlists.items.map { playlist ->
                playlist.copy(
                    rating = calculateRatingForPlaylist(
            ...
            val existingCourses =
                dao.getCoursesList(false, currentUid).associateBy { it.id }

            val entities = courses.map { searchCourse ->
                val existingCourse =
                    existingCourses[searchCourse.playlistId.playlistId]
    ...
    fun getCourseById(id: String): LiveData<CourseEntity> =
        dao.getCourseById(id, getUid())

    suspend fun getCourseByIdDirect(id: String): CourseEntity? =
        dao.getCourseByIdDirect(id, getUid())

    private suspend fun calculateRatingForPlaylist(
        playlistId: String
    ): Float? {
        return try {
            val playlistItems =
                RetrofitInstance.api.getPlaylistItems(
            ...
            val videoStats =
                RetrofitInstance.api.getVideoStats(
                    videoId = firstVideoId,
                    apiKey = apiKey
                )

            val stats =
                videoStats.items.firstOrNull()?.statistics
                    ?: return null
            ...
            val normalizedViews =
                (logViews / 6f).coerceIn(0f, 1f)

            val normalizedLikes =
                ((logLikes / 4f) + likeRatio).coerceIn(0f, 1f)

            val weightedScore =
                (normalizedViews * popularityWeight) +
                        (normalizedLikes * qualityWeight)
